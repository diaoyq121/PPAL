...
def create_dataset_csv_from_bids(bids_path: str):
    ...
    df = pd.DataFrame(columns=['subject', 'mri_path', 'diagnosis', 'age', 'cdr_global', 'MMSE'])
    subjects = layout.get_subjects()
    for i, subject in enumerate(subjects):
        # Get the sessions for the subject
        subject_sessions = layout.get_sessions(subject=subject)
        subject_session_file = layout.get(subject=subject, suffix='sessions')
        if not subject_session_file:
            continue

        subject_session_file = subject_session_file[0]
        ...
        subject_sessions_df = subject_session_file.get_df()[['session_id', 'diagnosis','age', 'cdr_global', 'MMSE']]
        ...
        for session in subject_sessions:
            ...
            # subject_smri_file = subject_smri_file + '.nii.gz'
            # Get the label associated to the session
            session_label = \
                subject_sessions_df[subject_sessions_df['session_id'] == 'ses-' + session]['diagnosis'].values[0]
            age = \
                subject_sessions_df[subject_sessions_df['session_id'] == 'ses-' + session]['age'].values[0]
            cdr_global = \
                subject_sessions_df[subject_sessions_df['session_id'] == 'ses-' + session]['cdr_global'].values[0]
            MMSE = \
                subject_sessions_df[subject_sessions_df['session_id'] == 'ses-' + session]['MMSE'].values[0]
            # Define the new row to be added
            new_row = {'subject': subject, 'mri_path': subject_smri_file, 'diagnosis': session_label, 'age':age, 'cdr_global':cdr_global, 'MMSE':MMSE}
            # Use the loc method to add the new row to the DataFrame
            df.loc[len(df)] = new_row
        # Print the progress
        print(f"Processed {i + 1} of {len(subjects)} subjects")
    # Save the dataframe to a csv file
    df.to_csv(os.path.join(bids_path, 'dataset_new2.csv'), index=False)
